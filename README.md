...
- **Outdoors: hours of the day and daily values.** Night frames, low sun and dew make outdoor series noisy in a way that isn't growth. Two report options help (they also work in `summary` and in `study.yaml` as `hours:` and `daily:`):
  - `--hours 10-14` fits only frames taken between those hours, local time. Set the site's `timezone:` in `config.yaml` (e.g. `Europe/Berlin`) so this holds when the analysis runs on a computer elsewhere. Windows past midnight such as `20-6` work too.
  - `--daily median|mean|p90` reduces each local day's usable frames to one value. `p90`, the 90th percentile, is the phenology-camera convention for greenness.
  - The daily value's uncertainty adds the frames' mean reported uncertainty (errors every frame of that day shares) to the bootstrap standard error of the day's statistic. In a simulation with shared daily errors, it covered the truth 90–98% of the time. A day with one frame keeps just the reported uncertainty.
  - `daily.csv` lists the days and `frame_flags.csv` still lists every frame, with an `outside_hours` column.
